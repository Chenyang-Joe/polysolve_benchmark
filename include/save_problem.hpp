...
void SerializeStiffnessMatrix(const polysolve::StiffnessMatrix& mat, const int dim, const int is_symmetric_positive_definite, const int is_sequence_of_problems, std::string filename) {
...
void DeserializeStiffnessMatrix(polysolve::StiffnessMatrix& m, int& dim, int& is_symmetric_positive_definite, int& is_sequence_of_problems, std::string filename) {
    std::fstream readFile;
    readFile.open(filename, std::ios::binary | std::ios::in);
    if(readFile.is_open())
    {
        ...
        // We already read one int32 as format check, but in int build this is actually rows
        int32_t rows_or_format = format_check;
        if (rows_or_format == -1) {
            // This shouldn't happen in int build - file format mismatch
            throw std::runtime_error("File was saved with large index format but trying to read with regular index format");
        }
